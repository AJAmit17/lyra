...
final class DecisionTests: XCTestCase {
    func testLargeRequestIncludesEveryActionWithinChoiceLimit() throws {
        ...
        let body = try TypeSafeClient.requestBody(context: context, candidates: candidates)
    ...
    func testCycleAsksOperationAndSpeculativeHeadsInOneRequest() throws {
        let state = TypeSafeClient.CycleState(goal: "search Rick Astley", dictation: nil, application: "Brave", window: "YouTube",
                                         elements: [TypeSafeClient.Element(index: 1, role: "textbox", label: "Search", value: "", place: "page", operations: ["CLICK", "TYPE_TEXT"])],
                                         available: TypeSafeClient.Available(apps: ["Finder"], folders: ["Desktop"], sites: [], menus: []), recentActions: [], previous: nil)
        let body = try TypeSafeClient.cycleBody(state: state, operations: ["CLICK": "Click", "DONE": "Done"], heads: ["click_target": ["e1": "[1] Search"], "type_target": [:]])
        let json = try XCTUnwrap(JSONSerialization.jsonObject(with: body) as? [String: Any])
        let questions = try XCTUnwrap(json["questions"] as? [String: [String: Any]])
    ...
    func testGroundingAsksOneTargetQuestionWithNone() throws {
        ...
        let context = TypeSafeClient.GroundingContext(step: step, goal: "play the first video", application: "Brave", window: "YouTube")
        let body = try TypeSafeClient.groundingBody(context: context, candidates: [Candidate(id: "control_1_Rick", label: "Rick", detail: "Item 1 of 2")])
        let json = try XCTUnwrap(JSONSerialization.jsonObject(with: body) as? [String: Any])
        let questions = try XCTUnwrap(json["questions"] as? [String: [String: Any]])
