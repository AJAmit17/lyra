...
final class CommandInputTests: XCTestCase {
    func testShortAppCommandsPreserveLiteralTextAndWebAddress() {
        ...
        let chain = CommandInput("Open Brave, go to x.com, and type in hello world from Lyra")
        XCTAssertEqual(chain.instructions, "Open Brave, go to x.com")
        XCTAssertEqual(chain.textToType, "hello world from Lyra")
        XCTAssertEqual(chain.websites.map(\.absoluteString), ["https://x.com"])
        XCTAssertEqual(CommandInput("Open Brave, go to x.com, type in hello world from Lyra, and post it").textToType, "hello world from Lyra")
