...
enum Desktop {
    ...
    private static let log = Logger(subsystem: "local.lyra", category: "desktop")
    ...
    static func capture(application: NSRunningApplication, command: String, dictation: String? = nil, includeMenus: Bool = true) async throws -> DesktopSnapshot {
        guard hasAccess else { throw DesktopError(message: "Enable Accessibility for Lyra in System Settings.") }
    ...
    private static func collect(application: NSRunningApplication, input: CommandInput, screens: [CGRect], includeMenus: Bool = true) throws -> DesktopSnapshot {
        ...
        // Why an input a person can see did not reach Lyra; logged with every capture.
        ...
        var pageItems = 0
        let began = Date()
        // Field metric for any app on any machine: what was walked, what reached Lyra, and why the rest did not.
        ...
        if !typingOnly {
            ...
            if let desktop = manager.urls(for: .desktopDirectory, in: .userDomainMask).first {
                do {
                    ...
                    folderAccessError = "Desktop folders could not be read. Allow Desktop folder access for Lyra in System Settings → Privacy & Security → Files and Folders."
                }
            }
        }
        ...
        if let window {
            ...
            func walk() {
                while let (control, parent, parentInPage, container, parentRegion, parentClip) = pending.popLast() {
                    ...
                    let info = read(control)
                    walked += 1
                    // Coverage audit: `defaults write local.lyra DumpTree -bool true` logs every walked node.
                    if dumpTree {
