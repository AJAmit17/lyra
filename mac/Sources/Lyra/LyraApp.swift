...
struct LyraApp: App {
    ...
    var body: some Scene {
        MenuBarExtra("Lyra", systemImage: model.isBusy ? "waveform" : "waveform.circle") {
            Text(model.headline)
            ...
            Divider()
            Text(model.handsFree ? "Tap ⌃⌥Space to speak" : "Hold ⌃⌥Space to speak")
            Button("Quit Lyra") { NSApplication.shared.terminate(nil) }.keyboardShortcut("q")
        }
    }
}
...
final class AppModel: ObservableObject {
    ...
    private let log = Logger(subsystem: "local.lyra", category: "status")
    @Published var headline = "Ready for a command" { didSet { log.notice("\(self.headline, privacy: .public) | \(self.detail, privacy: .public)") } }
    @Published var detail = "Press Control–Option–Space to speak. Escape cancels." { didSet { log.notice("  \(self.detail, privacy: .public)") } }
    ...
    /// Optional OpenRouter key: when present, the planner model turns the sentence into steps and Lyra grounds each one.
    private var plannerKey: String?
    @Published var hasPlannerKey = false
    /// Tap the shortcut to start and tap again to stop, instead of holding it down.
    @Published var handsFree = UserDefaults.standard.bool(forKey: "HandsFree") {
        didSet { UserDefaults.standard.set(handsFree, forKey: "HandsFree") }
    }
    /// Listen for the wake phrase whenever nothing else is going on.
    ...
    static let overlayDrop: CGFloat = 150
    /// True just after a status change, so the notch opens even when nothing is being spoken.
    ...
    func start() {
        ...
        commandObserver = DistributedNotificationCenter.default().addObserver(
            forName: Notification.Name("local.lyra.command"), object: nil, queue: .main
        ) { [weak self] notification in
        ...
        wake.onHeard = { [weak self] in
            guard let self, !capturing, !isBusy else { return }
            headline = "Heard “\(wake.phrase)”"
            beginSpeech()
        }
        // The wake listener owns the microphone only while nothing else needs it.
        speech.$isListening.sink { [weak self] _ in
            Task { @MainActor in self?.syncWake() }
        }.store(in: &subscriptions)
        $isBusy.sink { [weak self] _ in
            Task { @MainActor in self?.syncWake() }
        }.store(in: &subscriptions)
        hotKey.onPress = { [weak self] in
            guard let self else { return }
            if handsFree && capturing { endSpeech() } else { beginSpeech() }
        }
        ...
        hotKey.onCancel = { [weak self] in
            guard let self else { return }
            // Escape stops listening too: in hands-free mode that is the only way out besides the shortcut.
            if capturing || isBusy { cancel() }
    ...
    func savePlannerKey(_ value: String) {
        do {
            ...
            detail = "\(Planner.model) will plan multi-step commands; Lyra still chooses every action."
        } catch { fail(error.localizedDescription) }
    }
    ...
    private func prepare() -> NSRunningApplication? {
        refreshPermissions()
        guard !isLoadingKey else { fail("Approve Lyra's saved-key prompt in Keychain, then try again."); return nil }
        guard hasKey else { fail("Add your TypeSafe API key in Settings."); showSettings(); return nil }
        guard accessibilityAllowed else {
            fail("macOS has not recognised this app's Accessibility grant. If Lyra is already enabled, remove its old entry and add the current app again.")
            showSettings()
            return nil
        }
    ...
    func runTyped(_ text: String) {
        ...
        // Coverage audit of the window in front; reads only and never calls Lyra, so it needs Accessibility but not the API key.
        ...
        // Test hook for the target parser and its verbs, without Lyra: `/act 14` performs target [14] of the window in front.
    ...
    private func run(_ command: String, in first: NSRunningApplication, started: Date) {
        ...
        task = Task {
            ...
            do {
                ...
                headline = lastResult
                detail = steps.count > 1 ? "Done: \(steps.count) steps. \(shortcutHint) to speak again." : "\(shortcutHint) to speak again. Escape closes."
                timing = String(format: "%.2fs total · %.2fs plan · %.2fs decision · %d action%@", Date().timeIntervalSince(started), planSeconds, modelSeconds, actions, actions == 1 ? "" : "s")
                    ...
                    // Default: one Lyra request per cycle, lyra-ultrafast style. Code owns sequencing; Lyra picks operation and target.
                    ...
                    if case .completed(let result, let count) = outcome {
                        actions = count
                        headline = result
                        detail = count > 1 ? "Done in \(count) actions. \(shortcutHint) to speak again." : "\(shortcutHint) to speak again. Escape closes."
                    }
    ...
    /// Execute one planned step: deterministic kinds run in code; on-screen kinds are grounded by one narrow Lyra question.
    private func executePlanned(_ step: PlanStep, goal: String, app first: NSRunningApplication, cached: DesktopSnapshot?, label: String,
        ...
        while true {
            ...
            let context = TypeSafeClient.GroundingContext(step: step, goal: goal, application: name(app), window: current_.windowTitle)
            let began = Date()
            let decision = try await TypeSafeClient.ground(context: context, candidates: candidates, apiKey: key)
    ...
    /// lyra-ultrafast style loop: every cycle sends the current element table, the goal, the dictation and recent actions,
    /// and asks Lyra for one operation plus speculative targets in a single request. Code executes and checks freshness.
    private func runCycles(_ goal: String, app first: NSRunningApplication, generation current: UUID, modelSeconds: inout Double) async throws -> StepOutcome {
        ...
        // The text to type is chosen by Lyra as a first and a last word of the sentence (select, do not generate). The regex
        ...
        var recent: [TypeSafeClient.RecentAction] = []
        var noChange = 0
        var lastResult = "Done"
        ...
        // A stated count is arithmetic: Lyra says which step it belongs to, code repeats that step exactly.
        ...
        // A chain: the steps that worked in this window, kept as descriptions of their targets. When Lyra judges that the goal
        // wants them in every window, code repeats them in the other windows without asking Lyra again.
        var chain: [ChainStep] = []
        ...
        for cycle in 1...14 {
            ...
            if let last = recent.last, last.result.hasSuffix(noEffect), offered != lastOffered {
                recent[recent.count - 1] = TypeSafeClient.RecentAction(action: last.action, result: last.result.replacingOccurrences(of: noEffect, with: "the window's content changed"), screenChanged: false)
                noChange = 0
            ...
            var elements: [TypeSafeClient.Element] = []
            ...
            for candidate in offeredClicks + inputs {
                ...
                elements.append(TypeSafeClient.Element(index: meta.index, role: meta.role, label: candidate.label, value: meta.value, place: meta.place, operations: ops))
            }
            if !clicks.isEmpty {
            ...
            if !inputs.isEmpty {
                ...
                // The wanted input may not be on screen yet (a narrow page keeps its search box behind a button). Lyra can say so.
            ...
            let available = TypeSafeClient.Available(apps: apps.map(\.label), folders: folders.map(\.label), sites: sites.map(\.label), menus: menus.map(\.label))
            let windowCount = Desktop.windows(of: app).count
            let state = TypeSafeClient.CycleState(goal: goal, dictation: dictation, application: name(app), window: snapshot.windowTitle,
            ...
            let decision = try await TypeSafeClient.cycle(state: state, operations: operations, heads: heads, apiKey: key)
            modelSeconds += Date().timeIntervalSince(began)
            try Task.checkCancellation()
            guard generation == current, var op = decision.choice("operation") else { throw DecisionError.invalidResponse }
            // Dictated text pulls Lyra toward typing at once. When Lyra itself judges that the sentence first creates something new
            ...
            case "WAIT":
                try await Task.sleep(nanoseconds: 400_000_000)
                recent.append(TypeSafeClient.RecentAction(action: "WAIT", result: "waited 0.4s", screenChanged: false))
                continue
            default: break
            }
            if headName != nil && targetCandidate == nil {
                recent.append(TypeSafeClient.RecentAction(action: op.id, result: "no target offered", screenChanged: false))
                noChange += 1
            ...
            if op.id == "PRESS_RETURN", op.confidence < 0.5 {
                recent.append(TypeSafeClient.RecentAction(action: "PRESS_RETURN", result: "NOT performed: not sure enough that the goal asks for Return here", screenChanged: false))
                noChange += 1
            ...
            // Arranging is idempotent and was read back as done. Lyra cannot see window positions, so it may ask for the same
            ...
            do {
                ...
                recent.append(TypeSafeClient.RecentAction(action: "\(op.id) \(label)", result: "NOT performed: the control changed before it could be used", screenChanged: false))
                continue
            }
            guard generation == current else { return .stopped }
            ...
            recent.append(TypeSafeClient.RecentAction(action: "\(op.id) \(label)", result: result, screenChanged: after.title != titleBefore))
            lastResult = result
            priorCommand = goal
            ...
            // Lyra judged, in the same request, that this operation completes the goal; when the action visibly worked, a further
    ...
    /// Repeat a recorded chain in every other window of the app, without Lyra. It goes step by step across the windows, so the
    ...
    /// Fallback loop: run one command through capture → Lyra → execute until Lyra reports it complete.
    private func execute(_ command: String, dictation: String?, goal: String?, app first: NSRunningApplication, label: String,
        ...
        chain: while true {
            ...
            let beganModel = Date()
            let decision = try await TypeSafeClient.decide(context: context, candidates: snapshot.candidates, apiKey: key)
    ...
    /// Runs or stops the wake listener to match the current state. Called whenever that state moves.
    private func syncWake() {
        if wakeWordEnabled && setupComplete && !speech.isListening && !capturing && !isBusy { wake.start() }
        else { wake.stop() }
    }

    /// Click-to-talk: the notch's microphone button and the shortcut do the same thing.
    func toggleSpeech() {
        if capturing { endSpeech() } else { beginSpeech() }
    }

    /// How to start speaking, worded for the current mode.
    var shortcutHint: String { handsFree ? "Tap ⌃⌥Space" : "Hold ⌃⌥Space" }

    /// Stops listening and hands the transcript to the command path.
    private func endSpeech() {
        guard capturing else { return }
        releasedAt = Date()
        headline = "Finishing speech…"
        speech.finish()
    }
    ...
    func showVoiceWidget() {
        ...
        if !isBusy {
            ...
            detail = setupComplete ? (handsFree ? "Tap ⌃⌥Space to start, tap again to act." : "Hold ⌃⌥Space to speak. Release to act.") : "Open Settings from the Lyra menu bar icon."
            transcript = ""
            timing = ""
    ...
    private func showOverlay() {
        if overlay == nil {
            let panel = NSPanel(contentRect: NSRect(origin: .zero, size: Notch.size()),
                                styleMask: [.borderless, .nonactivatingPanel], backing: .buffered, defer: false)
            // Above the menu bar: the widget has to read as part of the notch, not as a window under it.
            ...
            overlay = panel
            Notch.place(panel, size: Notch.size())
        }
        systemAudio.start()
    ...
    /// Opens the notch for a moment so a status change is seen, then lets it close itself.
    private func peek() {
        peekTask?.cancel()
        isPeeking = true
        peekTask = Task {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 22) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                Text("With an OpenRouter key, \(Planner.model) turns each spoken command into ordered steps and Lyra chooses every on-screen action. Without it, Lyra alone handles single commands. Only the spoken text and app names are sent to OpenRouter.")
                    .font(.caption).foregroundStyle(.secondary)
            }
            VStack(alignment: .leading, spacing: 8) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                Text("3. Press ⌃⌥Space and speak").font(.headline)
                Toggle("Hands-free — tap to start, tap again to stop", isOn: $model.handsFree)
                    .toggleStyle(.switch).controlSize(.small)
                Toggle("Wake word — say “\(model.wake.phrase)” to start listening", isOn: $model.wakeWordEnabled)
                    .toggleStyle(.switch).controlSize(.small).disabled(!model.wake.isSupported)
                Text(model.wake.isSupported
                     ? "The wake listener runs on device only: nothing it hears is sent anywhere, and it stops while a command is running. It keeps the microphone open, so leave it off if you would rather it stay shut."
                     : "This Mac has no on-device speech model for your language, so the wake word is unavailable.")
                    .font(.caption).foregroundStyle(.secondary)
                Text(model.handsFree ? "Tap ⌃⌥Space, speak, tap again to act. Escape stops listening."
                                     : "Hold ⌃⌥Space, speak, release to act. Escape stops listening.")
...
private struct VoiceWidget: View {
    ...
    private var notchHeight: CGFloat { Notch.size().height }
    /// Open while there is something to see, and while the pointer rests on the notch.
    private var isOpen: Bool {
        isHovering || speech.isListening || model.isBusy || !model.transcript.isEmpty || model.isPeeking
    }

    private var hint: String {
        if speech.isListening { return model.handsFree ? "Tap ⌃+⌥+Space to send" : "Release to send" }
        return model.handsFree ? "Tap ⌃+⌥+Space to speak" : "Hold ⌃+⌥+Space to speak"
    }
                    ...
                    TimelineView(.animation(minimumInterval: 1.0 / 60)) { timeline in
                        Canvas { context, size in
                            ...
                            if let word = model.word {
                                field.spell(word)
                            } else if !speech.isListening && music.isPlaying {
                                field.equalise(music.bands, at: now)
                            } else {
                                field.rest(at: now, level: speech.isListening ? speech.audioLevel : 0)
                            }
            ...
            HStack(spacing: 6) {
                Button { model.toggleSpeech() } label: {
                    Image(systemName: speech.isListening ? "stop.circle.fill" : "mic.circle.fill")
                        .font(.system(size: 15))
                        .foregroundStyle(speech.isListening ? Color.red.opacity(0.9) : .white.opacity(0.55))
                }
                .buttonStyle(.plain)
                .accessibilityLabel(speech.isListening ? "Stop listening" : "Start listening")
                .help(speech.isListening ? "Stop listening and act" : "Click to talk")
                Text(hint).font(.system(size: 10, design: .monospaced)).foregroundStyle(.white.opacity(0.45))
            }
        }
...
private final class PixelField {
    private struct Pixel {
        ...
        /// `ta` is the target opacity; `alpha` eases towards it so pixels fade instead of blinking.
        var ta: Double = 1
        var alpha: Double = 0
        let seed: Double
    }
    ...
    func spell(_ newWord: String?) {
        guard newWord != word || equalising || resting else { return }
        word = newWord
        equalising = false
        resting = false
        var targets: [CGPoint] = []
    ...
    /// A single travelling wave across the middle: what the field shows with nothing to spell and no music.
    /// `level` is the live microphone level, so the wave swells while you speak.
    func rest(at time: Double, level: Double) {
        resting = true
        equalising = false
        word = nil
        let middle = bounds.height * 0.5
        let swell = 8 + level * 24
        for index in pixels.indices {
            guard index < Self.restDots else {
                // Everything else parks where it stands and fades out, so no loose pixels drift about.
                pixels[index].tx = pixels[index].x
                pixels[index].ty = pixels[index].y
                pixels[index].ta = 0
                continue
            }
            let position = Double(index) / Double(Self.restDots - 1)
            let phase = time * 2.4 - position * 8
            // A second, slower wave keeps the stroke from looking like a clock hand.
            let swayed = sin(phase) * 0.7 + sin(phase * 0.43 + 1.1) * 0.3
            // Tapered at both ends so the wave reads as one stroke rather than a band.
            let taper = sin(position * .pi)
            pixels[index].tx = 8 + position * (bounds.width - 16)
            pixels[index].ty = middle + swayed * swell * (0.25 + 0.75 * taper)
            pixels[index].ta = (0.25 + 0.6 * (0.5 + 0.5 * swayed)) * (0.3 + 0.7 * taper)
        }
    }
    private static let restDots = 150
    ...
    func step(to time: Double, level: Double) {
        ...
        for index in pixels.indices {
            ...
            if let tx = pixel.tx, let ty = pixel.ty {
                // The resting wave sets its own shape every frame; jitter there would only smear it.
                let shake = resting ? 0 : jitter
                pixel.x += (tx - pixel.x) * approach + sin(time * 21 + pixel.seed * 40) * shake
                pixel.y += (ty - pixel.y) * approach + cos(time * 17 + pixel.seed * 30) * shake
            }
            pixel.alpha += (pixel.ta - pixel.alpha) * approach
            pixels[index] = pixel
        }
    }
    ...
    func draw(in context: inout GraphicsContext) {
        for pixel in pixels {
            ...
            if equalising {
                ...
                context.fill(Path(ellipseIn: CGRect(x: pixel.x - 1.1, y: pixel.y - 1.1, width: 2.2, height: 2.2)),
